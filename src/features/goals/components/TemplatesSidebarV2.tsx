...
export function TemplatesSidebarV2({
  ...
  return (
    <div className="w-full md:w-[17em] md:shrink-0 md:self-stretch">
      <Card
        className={cn(
          "flex w-full flex-col overflow-hidden bg-background/95 shadow-[0_18px_40px_rgba(15,23,42,0.08)] backdrop-blur",
      ...
      >
        <CardHeader className="space-y-4 bg-gradient-to-b from-background to-surface/35 p-4 pb-4">
          <div className="flex items-center justify-between gap-3">
            <Badge
              variant="outline"
              className="inline-flex h-10 w-fit items-center gap-2 rounded-full border-transparent bg-surface/70 px-3.5 py-0 text-[11px] uppercase tracking-[0.22em]"
            >
            ...
            <Toggle
              ...
              className={cn(
                "h-10 w-10 shrink-0 rounded-full border-0 bg-surface/70 p-0 shadow-none hover:bg-surface",
                isEraserOn &&
                  "border border-rose-400/50 bg-rose-500/10 text-rose-600 hover:bg-rose-500/15 hover:text-rose-600"
              )}
            >
        ...
        <CardContent className="flex min-h-0 flex-1 flex-col overflow-hidden p-4">
          <Card className="rounded-2xl bg-[hsl(var(--primary)/0.08)] shadow-none">
          ...
          <Card className="mt-4 flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl bg-[hsl(var(--primary)/0.08)] shadow-none">
            <CardContent className="min-h-0 flex-1 p-0">
              <div className="flex h-full min-h-0 flex-col gap-2.5 overflow-y-auto p-4">
                ...
                {!isLoadingTemplates && filteredTemplates.length === 0 && (
                  <div className="rounded-2xl bg-background/70 px-3 py-4 text-sm text-muted-foreground">
                ...
                {!isLoadingTemplates &&
                  filteredTemplates.map((template) => (
                    <div
                      ...
                      className={cn(
                        "group w-full overflow-hidden rounded-2xl bg-input p-3 select-none shadow-none transition-colors",
                        eraserMode === "eraseOff"
                          ? "cursor-grab active:cursor-grabbing hover:border hover:border-border hover:bg-[hsl(var(--input-hover))]"
                          : "cursor-not-allowed opacity-70",
                        draggingTemplate?.id === template.id &&
                          draggingTemplate.kind === template.kind &&
                          "border border-primary shadow-[0_0_0_1px_hsl(var(--ring))]"
                      )}
                    >
