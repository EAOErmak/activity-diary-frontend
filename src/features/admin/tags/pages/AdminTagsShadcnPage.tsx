...
import { getAdminTagsQueryOptions } from "@/shared/lib/queryOptions";
...
export default function AdminTagsShadcnPage() {
  ...
  const { data = null, isPending: loading, error } = useQuery<Slice<Tag>, Error>(
    getAdminTagsQueryOptions(page, ADMIN_TAGS_PAGE_SIZE, debouncedQuery)
  );
  const tagsErrorMessage = error?.message ?? null;
  ...
  const statusMeta = useMemo(
    () => ({
      PROPOSED: {
        ...
        badgeClassName:
          "border-transparent bg-amber-500/10 text-amber-200",
      },
      APPROVED: {
      ...
      DEPRECATED: {
        ...
        badgeClassName:
          "border-transparent bg-slate-500/10 text-slate-200",
      },
    }),
    [t]
  );
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <DialogContent className="mb-0 w-[520px] max-w-[calc(100vw-1rem)] overflow-hidden rounded-[28px] bg-background p-0 shadow-[0_24px_80px_rgba(15,23,42,0.22)]">
          <DialogHeader className="px-4 py-3.5 text-left sm:px-5">
