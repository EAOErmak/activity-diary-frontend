import type { DiaryEntryView, EntryStatus } from "@/shared/types/diary";
...
export function getLeftBarColor(status: EntryStatus) {
  return STATUS_LEFT_BAR[status];
}

/* ===== LEFT BAR ===== */
export const STATUS_LEFT_BAR: Record<EntryStatus, string> = {
  PLANNED: "bg-plannedBorder",
  ACTIVE: "bg-activeBorder",
  OVERDUE: "bg-overdueBorder",
  FINISHED: "bg-winBorder",
  FAILED: "bg-loseBorder",
  DELETED: "bg-loseBorder",
};

export const STATUS_STYLES: Record<EntryStatus, string> = {
